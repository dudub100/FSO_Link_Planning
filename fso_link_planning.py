...
def get_closest_airport(lat, lon):
    """Downloads global airport DB, filters for METAR capabilities, and finds the closest."""
...
def fetch_metar_visibility(icao_code, years_back=3):
    """Fetches historical METAR visibility data from IEM ASOS archive."""
    ...
    try:
        ...
        if df.empty or 'vsby' not in df.columns:
            return None
        # Convert miles to km and drop NaNs
...
def get_itu_rain_rate(lat, lon, availability):
    """Calculates rain rate exceeded for a given percentage using ITU-R P.837."""
    p = 100.0 - availability 
    R = itur.models.itu837.rainfall_rate(lat, lon, p)
    return float(R.value)

def get_percentile_visibility(vis_array, availability):
    """Calculates visibility distance based on required availability percentile."""
...
def get_elevation_profile(lat1, lon1, lat2, lon2, num_points=50):
    """Fetches terrain elevation data."""
